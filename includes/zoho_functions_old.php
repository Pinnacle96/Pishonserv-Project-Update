...
function createZohoProperty($property_code, $title, $price, $location, $listing_type, $status, $type, $bedrooms, $bathrooms, $size, $description, $garage, $zoho_lead_id, $user_id)
{
    ...
    if (empty($zoho_lead_id) || empty($user_id) || empty($property_code)) {
        error_log($log_prefix . "Missing IDs: zoho_lead_id=$zoho_lead_id, user_id=$user_id, property_code=$property_code\n", 3, __DIR__ . '/../logs/zoho_debug.log');
    ...
    $product_data = [
        "data" => [[
            ...
            "Vendor_Name" => ["id" => $zoho_lead_id]
        ]]
    ];

    // === Check if product already exists ===
    $search_url = "https://www.zohoapis.com/crm/v2/Products/search?criteria=(Product_Code:equals:$property_code)";
    ...
    $deal_data = [
        "data" => [[
            ...
            "Type" => ucfirst($type),
            "Contact_Name" => ["id" => $zoho_lead_id], // Should be contact_id if available
...
//   // 🆕 Log all key request data
// error_log($log_prefix . "📤 Zoho API Endpoint: $product_url\n", 3, __DIR__ . '/../logs/zoho_debug.log');
// error_log($log_prefix . "🔐 Access Token (first 40): " . substr($access_token, 0, 40) . "...\n", 3, __DIR__ . '/../logs/zoho_debug.log');
// error_log($log_prefix . "📦 Request Headers:\n" . print_r($headers, true), 3, __DIR__ . '/../logs/zoho_debug.log');
// error_log($log_prefix . "📝 JSON Payload:\n" . json_encode($product_data, JSON_PRETTY_PRINT) . "\n", 3, __DIR__ . '/../logs/zoho_debug.log');
...
// // 🆕 Log full response
// error_log($log_prefix . "📥 Zoho Raw Response (HTTP $http_code):\n$product_response\n", 3, __DIR__ . '/../logs/zoho_debug.log');
