...
try {
    ...
    /* ---------- 7.  VALIDATION  ---------- */
    // ⭐ CHANGED: Added 'host' to valid roles
    $validRoles = ['buyer','agent','owner','hotel_owner','developer','host','admin','superadmin'];
    ...
    // ⭐ CHANGED: NIN is optional; only validate if provided
    if ($nin !== '' && !preg_match('/^[0-9]{11}$/', $nin)) {
        throw new Exception('If provided, NIN must be exactly 11 digits.');
    }
    ...
    // ⭐ CHANGED: include 'host' among roles that require MOU
    if (in_array($role, ['agent','owner','hotel_owner','developer','host'], true)) {
        ...
        $signed_name = $signedName;        // variable used in the MOU template
    ...
    if ($refParam !== '') {
        ...
        if ($refUser && (int)$refUser['id'] !== (int)$userId) {
            ...
            $conn->prepare('UPDATE users SET referrer_user_id = ? WHERE id = ?')->bind_param('ii', $referrerId, $userId);
            $upd = $conn->prepare('UPDATE users SET referrer_user_id = ? WHERE id = ?');
            $upd->bind_param('ii', $referrerId, $userId);
            $upd->execute();
            $upd->close();
            $context = in_array($role, ['agent','owner','hotel_owner','developer','host'], true) ? 'agent' : 'customer';
